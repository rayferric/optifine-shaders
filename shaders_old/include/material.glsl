...
MaterialMask decodeMask(in float value) {
	...
	mask.isSky         = bool((i >> 5) & 1);
	return mask;
}

MaterialMask genLitMask(in vec3 entity) {
	int id = int(entity.x + 0.5);
	MaterialMask mask;
	mask.isLit         = true;
...
MaterialMask genUnlitMask() {
	return MaterialMask(false, false, false, false, false, false);
}

MaterialMask genSkyMask() {
	return MaterialMask(false, false, false, false, false, true);
}
