...
vec3 softShadow(in vec3 worldPos, in vec3 normal, in vec3 lightDir, in bool fakeSss = false) {
	...
	float bias = fakeSss ? 0.001 : getShadowBias(normal, lightDir, distortionFactor);
	...
	for (int i = 0; i < SHADOW_DISTANCE_SAMPLES; i++) {
		vec2 unitOffset = hashToCircleOffset(frameTimeCounter * worldPos + float(i));
