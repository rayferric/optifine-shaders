...
#define BLOOM_MIN_THRESHOLD 0.5
#define BLOOM_MAX_THRESHOLD 1.5
#define BLOOM_MIN_SIZE   5
#define BLOOM_MAX_SIZE   21
...
vec3 writeBloomAtlas(in sampler2D colorTex, in vec2 screenPos) {
	...
	// Apply threshold
	vec3 color = texture2DLod(colorTex, screenPos, lod1).xyz * 3.0;
	return smoothstep(BLOOM_MIN_THRESHOLD, BLOOM_MAX_THRESHOLD, luminance(color)) * color;
}
...
vec3 blurBloomAtlas(in sampler2D atlas, in vec2 screenPos, in bool vertical) {
	...
	int size = int(mix(float(BLOOM_MIN_SIZE), float(BLOOM_MAX_SIZE), sizeMixFactor) + 0.5);
...
vec3 readBloomAtlas(in sampler2D atlas, in vec2 screenPos) {
	...
	for (int lod = 1; lod <= BLOOM_LEVELS; lod++) {
		...
		vec3 sample = texture2D(atlas, tilePos).xyz;
		color.xyz += sample * sample * weight;
		color.w   += weight;
	}
	return (color.xyz / color.w) * exp2(float(BLOOM_INTENSITY));
}
