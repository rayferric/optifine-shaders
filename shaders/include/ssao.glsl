...
float computeSSAO(in vec3 fragPos, in vec3 normal, in sampler2D depthTex) {
	...
	for (int i = 0; i < SSAO_SAMPLES; i++) {
		...
		float testDistance = -samplePos.z;
		float realDistance = -getFragPos(coord, depthTex).z;
		float rangeFactor = smoothstep(0.0, 1.0, SSAO_RADIUS / distance(realDistance, testDistance));
		aoStrength += float(realDistance < testDistance) * rangeFactor;
	}
